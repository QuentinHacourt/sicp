  ...
  (let ((s '()))
    ...
    (define (dispatch message)
      (cond ((eq? message 'push) push)
            ...
            ;; Toevoeging voor dodona tests
            ((eq? message 'empty?) (null? s))
...
;                              insts)
;                        labels)))))))
;; Einde continuatie-gebaseerd alternatief
...
(define geluk-machine
  (make-machine
    ...
    is-gelukkig
      ...
      (branch (label is-gelukkig-ja))
      (save cont)
      (assign cont (label is-gelukkig-na-kwadraten))
      (goto (label kwadrateer-cijfers))

    is-gelukkig-na-kwadraten
      (assign v1 (reg res))
      (restore cont)
      (goto (label is-gelukkig))
    ...
    kwadrateer-cijfers-iter
      (test (op <) (reg v1) (const 10))
      (branch (label kwadrateer-cijfers-end))
      (save v1)
      ...
      (goto (label kwadrateer-cijfers-iter))

    kwadrateer-cijfers-end
      (assign v1 (op *) (reg v1) (reg v1))
      (assign res (op +) (reg res) (reg v1))
      (goto (reg cont))
...
; (set-register-contents! geluk-machine 'v1 239)
; (start geluk-machine)
